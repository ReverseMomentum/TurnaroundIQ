...
"""
scrape_epl_multi.py
Premier League 2020/21-2025/26 from Understat.

Writes:
  stats/prem_20_25.csv
  stats/prem_events.csv
  data/ginf.csv
  data/events.csv

    pip install requests beautifulsoup4 pandas lxml
    python3 -u scrape_epl_multi.py
"""
...
STATS_COLS = [
    "fixture_id", "season", "matchweek", "date", "home_team", "away_team",
    ...
    "red_cards_home", "red_cards_away", "source",
]

EVENTS_COLS = [
    "event_id", "fixture_id", "season", "date", "home_team", "away_team",
    "minute", "event_team", "event_type", "scorer", "assist",
    "is_penalty", "is_own_goal", "score_after", "source",
]
...
def main():
    ...
    for year, label in SEASONS.items():
        print(f"\n=== {label} ===")
        html = fetch(LEAGUE_URL.format(year=year))
        matches = json_from_script(html, "datesData")
        print(f"  {len(matches)} matches on league page")
        ...
        for match in matches:
            ...
            xg = match.get("xG") or {}
            try:
                hg = int(goals.get("h") or 0)
                ag = int(goals.get("a") or 0)
            except (TypeError, ValueError):
                continue
            try:
                xgh = round(float(xg.get("h") or 0), 2)
                xga = round(float(xg.get("a") or 0), 2)
            except (TypeError, ValueError):
                xgh = xga = ""
            raw_dt = match.get("datetime") or match.get("date") or ""
            iso_date = ""
            uk_date = ""
            if raw_dt:
            ...
            if mid:
                try:
                    shots = flatten_shots(
                        json_from_script(fetch(MATCH_URL.format(mid=mid)), "shotsData")
                    )
                except Exception as exc:
            ...
            for shot in shots:
                ...
                event_rows.append({
                    ...
                    "minute": minute,
                    "event_team": team,
                    "event_type": "penalty" if is_pen else "own_goal" if is_og else "goal",
                    "scorer": scorer,
                    "assist": shot.get("player_assisted") or "",
                    "is_penalty": is_pen,
                    "is_own_goal": is_og,
                    "score_after": f"{sh}-{sa}",
                    "source": "understat",
    ...
    print("\nDone.")
    print(f"  {len(stats_rows)} fixtures -> stats/prem_20_25.csv")
    print(f"  {len(event_rows)} goals    -> stats/prem_events.csv")
    print("  pipeline copies            -> data/ginf.csv and data/events.csv")
