...
def update_live_team_stats():
    conn = get_db()
    migrate_live_columns(conn)
    teams = conn.execute("SELECT DISTINCT team FROM team_stats").fetchall()
    updated = 0
    ...
    for row in teams:
        ...
        home_rows = conn.execute(
            """
            SELECT home_2up, home_turnaround,
        ...
        away_rows = conn.execute(
            """
            SELECT away_2up, away_turnaround,
        ...
        two_up_leads = comeback_attempts = successful_comebacks = 0
        early_goals = early_concedes = first_leads = first_concedes = 0
        lead_games = retained_leads = 0
        half_diff_sum = second_half_diff_sum = 0
        ...
        for (
        ...
        ) in list(home_rows) + list(away_rows):
            if trigger:
                two_up_leads += 1
            if first_concede:
                comeback_attempts += 1
            ...
            if led:
                ...
                if (team_final or 0) > (opp_final or 0):
                    retained_leads += 1
            half_diff_sum += (fh_for or 0) - (fh_against or 0)
            second_half_diff_sum += (sh_for or 0) - (sh_against or 0)

        live_trigger_rate = pct(two_up_leads, matches_played)
        live_early_goal_rate = pct(early_goals, matches_played)
        live_early_concede_rate = pct(early_concedes, matches_played)
        live_first_lead_rate = pct(first_leads, matches_played)
        live_first_concede_rate = pct(first_concedes, matches_played)
        live_comeback_rate = pct(successful_comebacks, comeback_attempts)
        live_lead_retention_rate = pct(retained_leads, lead_games)
        live_first_half_goal_diff = round(half_diff_sum / matches_played, 3)
        live_second_half_goal_diff = round(second_half_diff_sum / matches_played, 3)

        existing = conn.execute(
            """
            SELECT historical_trigger_rate, early_goal_rate, early_concede_rate,
                   first_lead_rate, first_concede_rate, comeback_rate,
        ...
        (
            historical_trigger_rate, early_goal_rate, early_concede_rate,
            first_lead_rate, first_concede_rate, comeback_rate,
            lead_retention_rate, burnout_index,
        ) = existing if existing else (0, 0, 0, 0, 0, 0, 0, 0)
        ...
        live_burnout_index = round(
            live_early_goal_rate * (100 - live_lead_retention_rate) * live_trigger_rate / 10000,
            2,
        )
        trigger_rate_delta = round(live_trigger_rate - historical_trigger_rate, 2)
        early_goal_delta = round(live_early_goal_rate - early_goal_rate, 2)
        early_concede_delta = round(live_early_concede_rate - early_concede_rate, 2)
        first_lead_delta = round(live_first_lead_rate - first_lead_rate, 2)
        first_concede_delta = round(live_first_concede_rate - first_concede_rate, 2)
        comeback_delta = round(live_comeback_rate - comeback_rate, 2)
        lead_retention_delta = round(live_lead_retention_rate - lead_retention_rate, 2)
        burnout_delta = round(live_burnout_index - burnout_index, 2)
        ...
        conn.execute(
            ...
            (
                ...
                live_first_lead_rate, live_first_concede_rate, live_comeback_rate,
                ...
                abs(trigger_rate_delta), abs(lead_retention_delta),
    ...
    conn.commit()
    conn.close()
    print(f"{updated} teams updated with live + divergence stats")
